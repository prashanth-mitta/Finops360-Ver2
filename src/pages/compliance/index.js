...
import { useCompliance, useAuditLogs } from '../../services/finops';
...
export default function ComplianceModule({ userRole }) {
  const initialDeadlines = useCompliance();
  const auditLogs = useAuditLogs();
  const [tab,       setTab]       = useState('calendar');
  const [deadlines, setDeadlines] = useState(initialDeadlines);
  useEffect(() => { setDeadlines(initialDeadlines); }, [initialDeadlines]);
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50">
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {tab === 'audit'    && <AuditTrail logs={auditLogs} />}
        {tab === 'stats'    && <ComplianceStats deadlines={deadlines} logs={auditLogs} />}
      </div>
    </div>
  );
}
